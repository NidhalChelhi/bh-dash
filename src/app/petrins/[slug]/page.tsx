...
const PetrinPage: React.FC = () => {
  ...
  const [editedPetrin, setEditedPetrin] = useState<Petrin | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setEditedPetrin(response.data);
      } catch (error) {
        console.error("Error fetching petrin:", error);
      }
    };
  ...
  const handleInputChange = (
    ...
    setEditedPetrin((prevState: Petrin | null) => ({
      ...prevState!,
      [name]: value,
    }));
  ...
  const handleSubmit = async () => {
    if (
      !editedPetrin?.name ||
      !editedPetrin?.description ||
      !editedPetrin?.model ||
      !editedPetrin?.patteCapacity ||
      !editedPetrin?.farineCapacity ||
      !editedPetrin?.volume ||
      !editedPetrin?.puissance ||
      !editedPetrin?.poids ||
      !editedPetrin?.dimensions ||
      !editedPetrin?.images ||
      editedPetrin?.images.length === 0
    ) {
    ...
    try {
      await axios.put(
        process.env.NEXT_PUBLIC_BACKEND_BASE_URL +
          `/api/petrins/edit/${petrin?.slug}`,
        editedPetrin
      );
      setHasChanges(false); // Reset hasChanges after successful submission
      toast.success("Petrin updated successfully!");
    } catch (error) {
      console.error("Error updating petrin:", error);
      toast.error("Failed to update petrin. Please try again.");
    }
  };
        ...
        <div className="flex flex-col  w-full items-start justify-start gap-4">
          ...
          <div className="flex flex-col gap-1 w-full">
            ...
            <textarea
              ...
              name="description"
              value={editedPetrin?.description || ""}
              onChange={handleInputChange}
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            <CustomFormInput
              label="Modèle"
              name="model"
              value={editedPetrin?.model || ""}
              onChange={handleInputChange}
            />
            <CustomFormInput
              label="Capacité de Farine (KG)"
              name="farineCapacity"
              value={editedPetrin?.farineCapacity || ""}
              onChange={handleInputChange}
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full">
            <CustomFormInput
              label="Capacité de Pâte (KG)"
              name="patteCapacity"
              value={editedPetrin?.patteCapacity || ""}
              onChange={handleInputChange}
            />
            <CustomFormInput
              label="Volume du Bol (L)"
              name="volume"
              value={editedPetrin?.volume || ""}
              onChange={handleInputChange}
            />
          </div>
          ...
          <div className="flex items-center gap-4 w-full">
            ...
            <input
              type="checkbox"
              name="stock"
              checked={editedPetrin?.stock === "available"}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center justify-center gap-y-2 gap-x-2 w-full  ">
            {editedPetrin?.images.map((img, index) => (
              <div
                key={index}
